...
router.post('/create', auth, async (req, res) => {
  try {
    const { type, title, description, images, location, city, event_time, max_participants } = req.body;
    ...
    const [result] = await pool.query(
      'INSERT INTO invitations (user_id, type, title, description, images, location, city, event_time, max_participants) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [req.userId, type, title, description || '', imagesStr, location || '', city || '', event_time || null, max_participants || 10]
    );
...
router.get('/all', auth, async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT i.*, u.nickname, u.avatar, u.gender, u.age, u.city as user_city,
        (SELECT COUNT(*) FROM participants p WHERE p.invitation_id = i.id AND p.status = 1) as participant_count
      FROM invitations i
      LEFT JOIN users u ON i.user_id = u.id
      ${where}
...
router.post('/report', auth, async (req, res) => {
  try {
    const { invitation_id, reason } = req.body;
    if (!invitation_id || !reason) return res.json({ code: 400, msg: '请填写举报原因' });

    const [inv] = await pool.query('SELECT id FROM invitations WHERE id = ?', [invitation_id]);
    if (inv.length === 0) return res.json({ code: 404, msg: '邀约不存在' });

    await pool.query(
      'INSERT INTO reports (user_id, target_type, target_id, reason) VALUES (?, ?, ?, ?)',
      [req.userId, 'invitation', invitation_id, reason]
    );
...
router.get('/search', auth, async (req, res) => {
  try {
    ...
    if (!keyword || !keyword.trim()) return res.json({ code: 0, data: { list: [], total: 0, hasMore: false } });

    const offset = (parseInt(page) - 1) * parseInt(pageSize);
    const searchTerm = '%' + keyword.trim() + '%';

    const [total] = await pool.query(
      `SELECT COUNT(*) as count FROM invitations i WHERE i.status = 1 AND (i.title LIKE ? OR i.description LIKE ? OR i.location LIKE ?)`,
    ...
    const [rows] = await pool.query(
      ...
      FROM invitations i
      LEFT JOIN users u ON i.user_id = u.id
      WHERE i.status = 1 AND (i.title LIKE ? OR i.description LIKE ? OR i.location LIKE ?)
      ORDER BY i.created_at DESC
      LIMIT ? OFFSET ?`,
